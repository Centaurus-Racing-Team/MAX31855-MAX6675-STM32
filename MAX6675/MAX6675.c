...
float Max6675_Read_Temp_1(void){
	...
	HAL_GPIO_WritePin(SSPORT_1,SSPIN_1,GPIO_PIN_RESET);       // Low State for SPI Communication
	HAL_SPI_Receive(&hspi1,DATARX,1,50);                  // DATA Transfer
	HAL_GPIO_WritePin(SSPORT_1,SSPIN_1,GPIO_PIN_SET);         // High State for SPI Communication
...
float Max6675_Read_Temp_2(void){
	float Temp=0;                                         // Temperature Variable
	HAL_GPIO_WritePin(SSPORT_2,SSPIN_2,GPIO_PIN_RESET);       // Low State for SPI Communication
	HAL_SPI_Receive(&hspi1,DATARX,1,50);                  // DATA Transfer
	HAL_GPIO_WritePin(SSPORT_2,SSPIN_2,GPIO_PIN_SET);         // High State for SPI Communication
	TCF=(((DATARX[0]|(DATARX[1]<<8))>>2)& 0x0001);        // State of Connecting
	Temp=((((DATARX[0]|DATARX[1]<<8)))>>3);               // Temperature Data Extraction
	Temp*=0.25;                                           // Data to Centigrade Conversation
	HAL_Delay(250);                                       // Waits for Chip Ready(according to Datasheet, the max time for conversion is 220ms)
